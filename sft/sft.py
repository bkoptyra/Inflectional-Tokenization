...
def run(cfg: DictConfig):
    ...
    deepspeed_config = 'sft/conf_sft/deepspeed/base_config.json'
    ...
    accelerator.init_trackers(
        project_name="tokenization_sft_pllum",
        init_kwargs={
            "wandb": {
    ...
    if 'multiple_seeds' in cfg_primitive['custom'] and cfg_primitive['custom']['multiple_seeds']:
        ...
        for i in range(how_many):
            used_seed = seed + i
            cfg_primitive['training']['seed'] = used_seed
            ...
            if '/' != cfg_primitive['custom']['s3_rclone_output_dir'][-1]:
                cfg_primitive['custom']['s3_rclone_output_dir'] += '/'
            cfg_primitive['custom']['s3_rclone_output_dir'] += str(used_seed)
