...
from utils.evaluation_nkjp import nkjp_evaluate
...
def main(
    ...
    model.eval()
    if 'cv_test_split' in data_args:
        prefix = data_args['cv_test_split']
    else:
        prefix = 'test'
    scores = nkjp_evaluate(accelerator, model, tokenizer, model_args['model_max_length'], data_args['train_data_path'], prefix, int(custom_args['inference_batch_size']), conf_training_args['output_dir'], pickle_prefix=prefix)
    print(scores)
